...
`:latex: {src: "S = 8[sec]/3[sec] = 2.67"}` reizes.  
...
To cik efektīva varētu būt pāreja uz paralēlo skaitļošanu nosaka Amdāla (*Amdahl*) likums, pēc kura var izrēķināt maksimālo iespējamo skaitļošanas procesa laika saīsināšanu, jeb skaitļošanas procesa paātrinājumu (eng. _Speedup_) sadalītu uzdevumu sistēmā. Ir pieņemts šo skaitļošanas ātruma uzlabojuma koeficientu apzīmēt ar `:latex: {src: "S_{max}"}`, kuru var aprēķināt sekojoši:
...
Šajā formulā `:latex: {src: "p_f"}` ir parametrs, kas raksturo kādu daļu no aprēķiniem varam veikt paralēli. Šis parametrs ir bezmērvienibu lielums (_eng. Unitless_).  Piemēram, ja 75% no aprēķiniem varam veikt paralēli, proti, 
  `:latex: {src: "p_f=0,75"}` , tad
`:latex: {src: "S_{max}=1/(1-0,75)=4"}`.  Tas nozīmē, ka izmantojot paralēlo skaitļošanu aprēķinu metodi, kopējo skaitļošanas laiku varam samazināt 4 reizes.

<!--
![3.7](https://raw.githubusercontent.com/viktorszagorskis/hpc-pamati/main/pix/paralel07.png)
-->

:include-image: pix/amdahls.png {anchorId: "speedup",   align:"center",  title: "Skaitļošanas ātruma uzlabojums, ko panāk ar mezglu skaitu pieaugumu", scale:1}



Apkoposim galvenos ierobežojošosfaktorus, ka neļauj iegūt maksimālu efektu no paralēlās skaitļošanas metožu izmantošanas un tie ir:
...
**Anna viena pati** var saskaitīt 20 skaitļus 19 sekundēs, proti, `:latex: {src: "T_s=19 [sec]"}`. Ja viņa pieaicina palīgā Jāni, tad katram ir jāsaskaita 10 skaitļi un to var paveikt deviņās sekundēs. Jānis iegūto rezultātu nodod Annai un tas papildus prasa  organizatorisko laiku transportam  `:latex: {src: "0.1 [sec]"}`. Pēc tam, kad dati no Jāņa saņemti, Annai ir vajadzīga viena sekunde, lai  Jāņa reultātu pieskaitītu savam rezultātam. Līdz ar to, kopīgais aprēķinu laiks, sadalot uzdevumu uz diviem skaitļotājiem un pēc tam apkopojot rezultātu, būs:
...
Tagad, ja vēl palīgā pieaicinātu Pēteri un Laimu, tad darbu varētu sadalīt uz četriem (sk. attēlu zemāk).  Tādā gadījumā, uzdevums ir  - katram jāsaskaita pieci skaitļi un  noslēgumā Anna apkopo rezultātu. Katrs darbinieks datus nodod Annai. Nodotos rezultātus Anna secīgi liek klāt pie kopējās summas. Tādai operācijai vajag   `:latex: {src: " 0.1 [sec]"}`. Tādā gadījumā kopējais aprēķinu laiks būs:
...
*Sekojošā attēlā: 1 – Kopējā skaitļu kopa, 2- Sadalīšana, 3- Rezultāta sūtīšana Annai, 4-Anna apkopo. Attēlā ALICE vietā Anna. Tas pats attiecas arī uz pēc tam sekojošo attēlu.*

![3.8](https://raw.githubusercontent.com/viktorszagorskis/hpc-pamati/main/pix/paralel08.png)

Mēs varētu vēl palielināt darbinieku skaitu uz astoņiem, bet tad vairs nevaram vienmērīgi sadalīt darba slodzi (20/8-2,5). Tad labākajā variantā četri summē trīs skaitļus, bet citi četri – tikai divus. Visus darbus gan varēs pabeigt tikai tad, kad Anna saņems rezultātus no visiem un varēs saskaitīs tos kopā. Tiem darbiniekiem, kuriem jāsaskaita trīs skaitļi, vajadzēs vairāk laika aprēķiniem un tas nosaka paralēli veicamo darbu ilgumu. Kopējais aprēķinu laiks šajā gadījumā būs
